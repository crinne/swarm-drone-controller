...
func (s *fakeSpawner) Spawn(_ context.Context) (int, error) {
	s.callCount++
	return s.id, s.err
}

func TestSpawnReturnsCreatedIDWithoutPassword(t *testing.T) {
	...
	if spawner.callCount != 1 {
		t.Fatalf("spawn calls = %d, want 1", spawner.callCount)
	}
...
func TestCORSAllowsSwarmUI(t *testing.T) {
	...
	if got := res.Header().Get("Access-Control-Allow-Headers"); got != "Content-Type" {
		t.Fatalf("cors headers = %q, want Content-Type", got)
	}
